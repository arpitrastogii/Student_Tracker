 ...
 */
@WebServlet("/OpenAssignmentSolutionFile")
public class OpenAssignmentSolutionFile extends HttpServlet {
	...
	try {
		...
		String sql="select solutionfile from ASSIGNMENT_SOL where solution_id="+solution_id;
		ResultSet rs=stmt.executeQuery(sql);
		if(rs.next()){
			byte[] pdfData=rs.getBytes(1);
			request.setAttribute("pdfData", pdfData);
			request.setAttribute("solution_id", solution_id);
			if(!who.equals("student")){
	        request.getRequestDispatcher("MarkAsViewedSolution").include(request, response);	  
			}
